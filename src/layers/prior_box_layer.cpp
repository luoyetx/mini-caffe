...
void PriorBoxLayer::LayerSetUp(const vector<Blob*>& bottom,
...
void PriorBoxLayer::Reshape(const vector<Blob*>& bottom,
                            const vector<Blob*>& top) {
...
void PriorBoxLayer::Forward_cpu(const vector<Blob*>& bottom,
                                const vector<Blob*>& top) {
  const int layer_width = bottom[0]->width();
  const int layer_height = bottom[0]->height();
  ...
  if (clip_) {
    for (int d = 0; d < dim; ++d) {
      top_data[d] = std::min<real_t>(std::max<real_t>(top_data[d], 0.), 1.);
    }
  }
  // set the variance.
  top_data += top[0]->offset(0, 1);
  if (variance_.size() == 1) {
    caffe_set(dim, static_cast<real_t>(variance_[0]), top_data);
  } else {
    int count = 0;
    for (int h = 0; h < layer_height; ++h) {
      for (int w = 0; w < layer_width; ++w) {
